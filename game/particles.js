class Particle {
    ...
    edges(){
        if (this.position.x > screenRes[0] - this.r) {
            this.position.x = screenRes[0] - this.r;
            this.velocity.x *= -1;
    ...
    remove(particle, index) {
        if (particle.position.y >= screenRes[1]*0.97 - particle.r) { //bottom
            Composite.remove(game.engine.world, particles[index].body)
